...
import {
    DeleteOutlined, EyeInvisibleOutlined, LockOutlined, CopyOutlined, BlockOutlined, UpOutlined, DownOutlined, UndoOutlined,
    RedoOutlined,
} from '@ant-design/icons'
import { useDispatch } from "react-redux";
import {
    removeSelectedComponent, changeComponentHidden, toggleComponentLocked
    , copySelectedComponent, pasteCopiedComponent, moveComponent
...
const EditToolbar = () => {
    ...
    function handleDelete() {
        dispacth(removeSelectedComponent())
    }

    // 隐藏组件
    function handleHidden() {
        dispacth(changeComponentHidden({ fe_id: selectedId, isHidden: true }))
    }

    // 锁定组件
    function handleLock() {
        dispacth(toggleComponentLocked({ fe_id: selectedId }))
    }

    // 复制
    function copy() {
        dispacth(copySelectedComponent())
    }

    // 粘贴
    function paste() {
        dispacth(pasteCopiedComponent())
    }

    // 上移
    function moveUp() {
        if (isFirst) return
        dispacth(moveComponent({ oldIndex: selectedIndex, newIndex: selectedIndex - 1 }))
    }
    ...
    // 撤销
    function undo() {
        dispacth(UndoActionCreators.undo())
    }

    // 重做
    function redo() {
        dispacth(UndoActionCreators.redo())
    }
